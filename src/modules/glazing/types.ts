...
/** Тип открывания створки или заполнение ячейки. */
export type SashType =
  | 'fixed'             // глухая (не открывается)
  | 'sandwich'          // сэндвич-панель (непрозрачная, вместо стекла)
...
export interface Frame {
  ...
  /**
   * Смещение нижнего края рамы от низа сегмента, мм. По умолчанию 0
   * (рама стоит на полу). Если > 0, рама «висит» — например окно над
   * выступом в балконном блоке (bottomOffset = 800 → окно начинается
   * на 800мм выше пола).
   */
  bottomOffset?: number;
  /**
   * Смещение верхнего края рамы от верха сегмента, мм. По умолчанию 0
   * (рама касается потолка). Используется когда нужен зазор сверху —
   * например окно ниже потолка с отступом для воздуховода.
   */
  topOffset?: number;
  /**
...
/**
 * Тип соединителя между рамами в одном сегменте:
 *   • 'bone' — массивная кость, толстая темно-серая планка (50мм визуально)
 *   • 'connector' — тонкая стыковочная планка (20мм визуально),
 *     используется в балконных блоках и для удобства транспортировки
 *     больших балконов (рама делится на части и собирается на месте)
 */
export type JoinType = 'bone' | 'connector';

/**
 * Кость / соединитель — вертикальный профиль между двумя рамами
 * в одной фасадной плоскости. Не делит раму, а стоит МЕЖДУ рамами.
 *
 * Тип определяет визуальное представление и функциональность:
 *   • bone — усиленная массивная кость (для конструкций которые нельзя сваривать)
 *   • connector — тонкая стыковочная планка (для разделения рам на транспортируемые части)
 */
export interface Bone {
  id: string;
  /** Индекс рамы слева в массиве segment.frames, после которой идёт элемент. */
  afterFrameIndex: number;
  /** ID материала из справочника. */
  materialId?: string;
  /** Тип соединения. По умолчанию 'bone' для совместимости со старыми данными. */
  type?: JoinType;
}
...
export interface GlazingProject {
  ...
  /**
   * Тип конструкции — для фильтрации в попапе создания и для будущих
   * пользовательских шаблонов. По умолчанию 'window'.
   */
